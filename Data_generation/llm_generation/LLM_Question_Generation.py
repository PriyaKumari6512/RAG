# """
# Automatic Question Generation for RAG Evaluation
# Extracts content from PDFs (text, tables, images) and generates diverse questions
# using Bosch Azure OpenAI GPT-4o-mini
# """
...
# # ==================================================
# # BOSCH AZURE OPENAI - GPT-4O-MINI
# # ==================================================
...
#     """Generate diverse evaluation questions using Bosch GPT-4o-mini"""
...
#         """Call Bosch Azure OpenAI GPT-4o-mini"""
#         headers = {
...
#         """Generate diverse questions from document content"""
        
#         # Prepare document text (limit to avoid token limits)
#         doc_text = content["full_text"][:15000]  # Limit context
        
#         prompt = f"""You are an expert at creating evaluation questions for Retrieval-Augmented Generation (RAG) systems.

# Given the following document content, generate EXACTLY {num_questions} diverse, high-quality questions for RAG evaluation.

# DOCUMENT CONTENT:
# {doc_text}

# REQUIREMENTS:
# 1. Generate EXACTLY {num_questions} questions covering different question types:
#    - Factoid questions (who, what, when, where)
#    - Definition questions (define, explain terminology)
#    - Numeric questions (numbers, dates, percentages, statistics)
#    - List questions (enumerate multiple items)
#    - Reasoning questions (why, how, explain process)
#    - Comparison questions (if document has multiple concepts)

# 2. Questions should:
#    - Be answerable ONLY from the provided document
...
#    - The question text
#    - A concise, accurate answer (2-3 sentences max)
#    - Question type (factoid/definition/numeric/list/reasoning/comparison/boolean)
#    - Difficulty level (easy/medium/hard)
#    - Approximate page number or section where answer is found

# OUTPUT FORMAT (JSON):
# Return ONLY a valid JSON array of questions, nothing else. Format:
...
#         try:
#             response = self.call_llm(prompt, temperature=0.7, max_tokens=2000)
            
#             # Try to parse JSON response
#             # Sometimes LLM adds markdown formatting, clean it
...
#                 # Extract page number if not present
#                 if "page_reference" not in q or not q["page_reference"]:
#                     q["page_reference"] = "Page 1"
            
#             return questions
            
#         except json.JSONDecodeError as e:
#             print(f"Failed to parse JSON response for {doc_id}: {e}")
...
#     print("="*80)
#     print("RAG EVALUATION QUESTION GENERATOR")
#     print("="*80)
...
#     generator = QuestionGenerator(GPT4O_MINI_ENDPOINT, API_KEY)
...
#             print(f"  ??  Failed to extract content, trying PyPDF2...")
...
#         # Generate questions
#         print(f"  ? Generating {questions_per_doc} questions using GPT-4o-mini...")
...
#     OUTPUT_PATH = "/home/olj3kor/praveen/RAG_work/evaluation_questions.json"
#     QUESTIONS_PER_DOC = 8  # Generate 8 questions per document
...
class QuestionGenerator:
    """Generate diverse evaluation questions using Bosch GPT-4o-mini with quality verification"""
    ...
    def call_llm(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2000, 
        ...
        """Call Bosch Azure OpenAI GPT-4o-mini with retry logic"""
        headers = {
            "genaiplatform-farm-subscription-key": self.api_key,
            "Content-Type": "application/json"
        }
...
2. QUESTION QUALITY STANDARDS:
   ...
   - Questions should vary in difficulty (easy: 20%, medium: 50%, hard: 30%)
...
def process_dataset(dataset_path: str, output_path: str, questions_per_doc: int = 8):
    ...
    print("="*80)
    print("RAG EVALUATION QUESTION GENERATOR (IMPROVED)")
    print("="*80)
    print(f"Dataset Path: {dataset_path}")
    print(f"Questions per Document: {questions_per_doc}")
    print(f"Output: {output_path}")
...
if __name__ == "__main__":
    # Configuration
    DATASET_PATH = "/home/olj3kor/praveen/RAG_work/DATASET/"
    OUTPUT_PATH = "/home/olj3kor/praveen/RAG_work/evaluation_questions_improved.json"
    QUESTIONS_PER_DOC = 1  # Generate 8 questions per document
